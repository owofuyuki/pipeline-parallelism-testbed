...
if __name__ == "__main__":
    ...
    S_L = utils.all_partition_case(s=2, num_layer=len(size))
    S_total = []
    for cut_layer in S_L:
        G = nx.Graph()

        # initial device with speed argument
        G.add_nodes_from([('A', {'speed': 2.75408874427}), ('B', {'speed': 0.34690411062}), ('C', {'speed': 1})])
        ...
        G.nodes['B']['end'] = cut_layer[1]
        G.nodes['B']['e'] = 1
        G.nodes['C']['start'] = cut_layer[1]
        G.nodes['C']['end'] = -1
        G.nodes['C']['e'] = 1

        # initial edge with speed (BW in MBps)
        G.add_edge('A', 'B', speed=3375000*2)
        G.add_edge('B', 'C', speed=3375000*2)
