...
class Shard1(nn.Module):
    ...
    def forward(self, x_rref):
        ...
        with self._lock:
            print("Start 1 " + str(self.c))
            out = self.conv1(x)
            out = self.trans1(self.dense1(out))
            out = self.trans2(self.dense2(out))
            print("End 1 " + str(self.c))
        self.c += 1
        return out.cpu()
...
class Shard2(nn.Module):
    ...
    def forward(self, x_rref):
        x = x_rref.to(self.device)
        with self._lock:
            print("Start 2 " + str(self.c))
            out = self.trans3(self.dense3(x))
            out = self.dense4(out)
            out = F.avg_pool2d(F.relu(self.bn(out)), 4)
            out = out.view(out.size(0), -1)
            out = self.linear(out)
            print("End 2 " + str(self.c))
        self.c += 1
        return out.cpu()
...
class DistNet(nn.Module):
    ...
    def forward(self, xs):
        ...
        x1_rref = RRef(xs)
        x2_rref = self.p_rref[0].rpc_sync().forward(x1_rref)
        x3_fut = self.p_rref[1].rpc_async().forward(x2_rref)
...
def run_master(split, world_size):
    ...
    def train():
        ...
        for data, target in tqdm(train_loader):
            # for i in range(thr):
            #     threading.Thread(target=f, args=(datas[i], targets[i])).start()
            # f(datas[i], targets[i])
            # time.sleep(2)
            f(data, target)
...
def run_worker(rank, world_size, num_split):
    options = rpc.TensorPipeRpcBackendOptions(num_worker_threads=4, rpc_timeout=30000)
